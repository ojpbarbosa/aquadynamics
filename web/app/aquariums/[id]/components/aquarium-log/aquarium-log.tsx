...
export default function AquariumLog({ aquariumId, logs, setLogs }: AquariumLogProps) {
  ...
  const temperatureData = getTemperatureData(logs[logs.length - 1].temperature)
  const pHData = getPHData(logs[logs.length - 1].pH)
    ...
    <>
      <div className="flex flex-col gap-y-2 text-neutral-500">
        ...
        <p className="text-lg">
          ...
          <span className={`text-${temperatureData.color} transition-colors duration-[2s]`}>
      ...
      <div className="flex flex-col gap-y-2 text-neutral-500">
        ...
        <p className="text-lg">
          O pH da água está{' '}
          <span className={`text-${pHData.color} transition-colors duration-[2s]`}>
